...
/*
1. what collection api doing internally while identifying duplicates. It is 
maintaining different buckets. to identify duplicates it is
maintaining hashbuckets. 
2. it is only calling hashcode method getting a hash number choosing one bucket for that hash numbe. 
3. storing the first element inside that bucket. So first
...
bucket. That bucket is labeled with hash number. 
2. while addding second element e2 again calling hashcode method getting hash
...
not adding only e1 is adding succesfuly.
e2 is a duplicate to e1.while adding e3 again it is calling hashcode method, definately we will be getting a different hash number, so if it is a
...
new hash number. So e3 added successfully. 
3. while adding e4 again calling hashcode method getting a hash number this hash number is already allocated to second bucket. Now it is calling
...
e4 element is not adding.
4. e5 is calling hashcode and e5 is same
...
contains 1 element. 
5. Then e6 same hash number same bucket,
...
e6 both are same returning true. 
6. for e7 we get different hash number and third bucket is allocating, there
...
element in the new bucket. 
7. e8 is having the same hash number 
...
in the third bucket. 
8. e9 is having the same hash number and
...
successfuly.
9. e10 has entirely new hash number and choosing
